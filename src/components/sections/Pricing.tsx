  ...
  return (
    <section id="pricing" className="relative py-24 bg-[#000] overflow-hidden" aria-labelledby="pricing-title">
      ...
      <div className="section-container relative z-10">
        ...
        {/* Toggle pill deslizante */}
        <div className="flex items-center justify-center mb-14">
          <div
            role="group"
            aria-label="Ciclo de facturación"
            className="relative flex items-center bg-bg-card border border-[var(--border-mid)] rounded-full p-1 gap-1"
          >
            <motion.div
              className="absolute top-1 bottom-1 rounded-full bg-grad-brand shadow-brand pointer-events-none"
              animate={{
            ...
            <button
              ...
              className={`relative z-10 px-5 py-2 rounded-full text-sm font-semibold transition-colors duration-200 ${
                cycle === 'monthly' ? 'text-white' : 'text-text-secondary hover:text-white'
              }`}
            >
              Mensual
            </button>
            <button
              ...
              className={`relative z-10 px-5 py-2 rounded-full text-sm font-semibold transition-colors duration-200 flex items-center gap-2 ${
                cycle === 'annual' ? 'text-white' : 'text-text-secondary hover:text-white'
              }`}
            >
              Anual
          ...
          {PRICING_PLANS.map((plan, i) => {
            ...
            return (
              <motion.div
                ...
                className={[
                  'h-full w-full md:w-[350px] lg:w-[380px]',
                  plan.featured ? 'gradient-border shadow-glow md:-translate-y-2' : '',
                ].join(' ')}
              >
                <CardTilt
                  className={[
                    'card-neon-border relative h-full flex flex-col gap-6 p-8 transition-all duration-250',
                    plan.featured
                      ? 'bg-gradient-to-br from-navy-brand/30 to-blue-action/10 rounded-[calc(var(--r-xl)-1px)]'
                      : 'rounded-[var(--r-xl)] bg-bg-card',
                  ].join(' ')}
                >
                  {/* Badge animado */}
                  {plan.badge && (
                    <span className="badge-shimmer animate-badge-pulse absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-xs font-semibold bg-grad-brand text-white shadow-brand whitespace-nowrap">
                      {plan.badge}
                    </span>
                  )}
